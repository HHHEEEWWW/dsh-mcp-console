...
/** Cap for waiting on a fiber's deferred teardown before giving up (ms). */
const DISPOSE_TIMEOUT_MS = 60_000
...
export function apply(ctx) {
  ...
  /**
   * Per-server serialization queue. Reconnect / disconnect / reload races are
   * the root cause of `serverName already in use`: an mcp-client fiber that is
   * still loading (e.g. a slow first npx download) releases its serverName
   * reservation only AFTER Cordis finishes its DEFERRED teardown, so a
   * re-create issued in that window collides with the still-reserved name and
   * leaves an untracked fiber holding the name forever. Every start/stop for
   * the same server therefore runs strictly in order.
   */
  const ops = new Map()

  function enqueue(serverId, fn) {
    const prev = ops.get(serverId) || Promise.resolve()
    const task = prev.then(fn).catch((e) => ctx.logger.warn(`[mcp-console] op for "${serverId}" failed: ${e?.message || e}`))
    ops.set(serverId, task)
    return task
  }

  /** Dispose an mcp-client fiber and WAIT for its teardown to finish, so its
   *  serverName reservation is released before a re-create races it. Cordis
   *  defers teardown while the plugin's async apply is still loading;
   *  fiber.dispose() returns the pending teardown promise in that case.
   *  Bounded, so a hung connection cannot block reconnect forever. */
  ...
  function startServer(server) {
    return enqueue(server.id, async () => {
      // Fully tear down the previous instance BEFORE creating a new one: the
      // bridge releases its serverName reservation only once the old fiber's
      // (possibly deferred) teardown actually finished.
      const existing = live.get(server.id)
      if (existing?.fiber) await disposeFiber(existing.fiber)
      ...
      try {
        ...
        if (entry.fiber) {
          await disposeFiber(entry.fiber)
          entry.fiber = null
        }
      }
    })
  ...
  return async () => {
    await Promise.all([...live.keys()].map((id) => stopServer(id)))
